            ...
            <div className="flex-1 max-w-4xl">
                ...
                {cartArry.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        <div className="flex items-center md:gap-6 gap-3">
                            <div className="cursor-pointer w-24 h-24 flex items-center justify-center border border-gray-300 rounded overflow-hidden">
                                <img
                                    ...
                                    src={product.image?.[0] || 'default-image.jpg'} // Optional chaining for image
                                    alt={product.name || 'Product Image'} // Fallback alt text if name is missing
                                />
                            </div>
                            <div>
                                <p className="hidden md:block font-semibold">{product.name}</p>
                        ...
                        <p className="text-center">{currency}{product.offerPrice * product.quantity}</p> {/* Safe access to offerPrice */}
                        <button onClick={() => removeFromCart(product._id)} className="cursor-pointer mx-auto">
                            <img src={assets.remove_icon} alt="remove" className="inline-block w-6 h-6" />
                        </button>
                    </div>))
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                {addresses.map((address, index) => (
                                    <p key={index} onClick={() => { setSelectedShowAddress(address), setShowAddress(false) }} className="text-gray-500 p-2 hover:bg-gray-100">
                                        {address.street}, {address.city}, {address.state}, {address.country}
                                    </p>
                                ))}
                                <p onClick={() => navigate("/add-address")} className="text-primary text-center cursor-pointer p-2 hover:bg-primary/10">
